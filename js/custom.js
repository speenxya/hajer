...
// }
	...
	function inputCounter(){
	    if ($j(".input-counter").length > 0) {
	        ...
	        $j('.minus-btn').click(function () {
											 			var maxx=$j(".input-counter").attr("maxx")
	...
	jQuery(function($) {
		  if ($j("#off-canvas-menu").length > 0) {
		   ...
		   if ( jQuery( ".off-canvas-menu-toggle" ).length ) {
	  ...
	  if($cboxClose.length && window.innerWidth > 1024 ) {
	    ...
	    $j("#off-canvas-menu .expander-list").find("ul").hide().end().find(" .expander").text("+").end().find(".active").each(function() {
	        ...
	        $jthis.click(function() {
	            if ($jul.css("display") ==
	                "block") $jul.slideUp(100);
	            else $jul.slideDown(100);
	...
	// Product Carousel
    	...
    	function productCarousel_sec(carousel,numberXl,numberLg,numberMd,numberSm,numberXs) {
...
//	$j('.stuck-nav').html($j(window).scrollTop())
 // $j(window).scrollTop() > HeaderTop ? $j('.stuck-nav').addClass('fixedbar') : $j('.stuck-nav').removeClass('fixedbar');
  $j(window).scrollTop() > 50 ? $j('.stuck-nav').addClass('fixedbar') : $j('.stuck-nav').removeClass('fixedbar');
}
function stickNav() {
...
//=========== top menu(hover)
	...
	$j('.nav.navbar-nav >li').hover(function(){
		...
		$j(this).find('.menuhoverr').show();
	},function(){
		$j(this).removeClass('hover');
		$j(this).find('.menuhoverr').hide();
	})
