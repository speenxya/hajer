...
//error_reporting(E_ALL ^ E_NOTICE);	
	...
	//User Permissions
     if($_SESSION['m_delete']=="1"){
	define('CONFIG_OPTIONS_DELETE',true); //disable to delete folder
	 }else{
		 define('CONFIG_OPTIONS_DELETE',false); //disable to delete folder
	 }
	 if($_SESSION['m_cut']=="1"){
	define('CONFIG_OPTIONS_CUT', true);	//disalbe to cut a file/folder
	}else{
		define('CONFIG_OPTIONS_CUT', false);	//disalbe to cut a file/folder
	 }
	if($_SESSION['m_copy']=="1"){
	define('CONFIG_OPTIONS_COPY', true);	//disable to copy a file/folder
	}else{
		define('CONFIG_OPTIONS_COPY', false);	//disable to copy a file/folder
	 }
	if($_SESSION['m_newfolder']=="1"){
	define('CONFIG_OPTIONS_NEWFOLDER', true); //disable to create new folder 
	}else{
		define('CONFIG_OPTIONS_NEWFOLDER', false); //disable to create new folder 
	 }
	if($_SESSION['m_rename']=="1"){
	define('CONFIG_OPTIONS_RENAME', true); //disable to rename the file/folder
	}else{
		define('CONFIG_OPTIONS_RENAME', false); //disable to rename the file/folder
	 }
	if($_SESSION['m_upload']=="1"){
	define('CONFIG_OPTIONS_UPLOAD', true); //disable to upload the file
	}else{
		define('CONFIG_OPTIONS_UPLOAD', false); //disable to upload the file
	 }
	if($_SESSION['m_edit']=="1"){
	...
	if($_SESSION['m_search']=="1"){
	define('CONFIG_OPTIONS_SEARCH', true); //disable to search documents
	}else{
		define('CONFIG_OPTIONS_SEARCH', false); //disable to search documents
	 }
	//FILESYSTEM CONFIG
		/*
		...
		*/
	define('CONFIG_SYS_DEFAULT_PATH', $_SESSION['m_path']); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', $_SESSION['m_path']);	//accept relative path only
	...
	define('CONFIG_SYS_EXC_DIR_PATTERN', 'filemanager'); //will prevent listing of folders with such pattern(s). separated by , if multiple
	define('CONFIG_SYS_INC_FILE_PATTERN', ''); //force listing of fiels with such pattern(s). separated by , if multiple
	define('CONFIG_SYS_EXC_FILE_PATTERN', 'index.html,.htaccess,Thumbs.db'); //will prevent listing of files with such pattern(s). separated by , if multiple
	...
	define('CONFIG_UPLOAD_VALID_EXTS', $_SESSION['m_valid']); // 
