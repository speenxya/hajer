      ...
      if (bounds){
       map.fitBounds(bounds);
              ...
              <div class="col-sm-12">
               ...
               <input class="btn btn-primary" type="button" value="Add Address" onClick="window.location='addresses.php'">
               <br /><br />
              </div>
               <div class="col-sm-12">
              <?php $phh=$con->query("select * from addresses left join access on accessid=idaccess where idaccess='".$_SESSION['hj_id']."'");
			   while($ph=$con->fetch_array($phh)){?>
                 <div><?php echo $ph['addressesname']?>
                  (<a href="addresses?code=<?php echo $ph['addressesid']?>">Edit</a> -  <a href="editprofile.php?deleteaddress=<?php echo $ph['addressesid']?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a>)</div>
               <?php }?>
              </div>
