...
if(!isset($_SESSION['hj_id'])){
	echo changelocation("cart");
			...
			if(@$_SESSION['subtotal']<"10"){
				...
				$msg="<font class='itsnotok'>يجب ألا يكون المنتج الإجمالي أقل من 10</font>";
			}else{
...
if(isset($_POST['vouchersend'])){
	...
	if($con->num_rows($d)==0){
		$msgv="كود الخصم المدخل لا ينطبق على المنتجات المختارة.";
	}else{
...
<div class="breadcrumbs">
        ...
        <table style="text-align:centerborder-collapse:collapse;width:100%">
         <tr>
           <td nowrap="nowrap" width="21%" class="smallmobile hideinmobile" style="text-align:center;background-color:#1a3282;color:#fff;border:1px solid #cccccc;padding:10px"><strong>الصورة</strong></td>
           <td nowrap="nowrap" width="32%" class="smallmobile" style="text-align:center;background-color:#1a3282;color:#fff;border:1px solid #cccccc;padding:10px"><strong>المنتج</strong></td>
           <td nowrap="nowrap" width="7%" class="smallmobile" style="text-align:center;background-color:#1a3282;color:#fff;border:1px solid #cccccc;padding:10px"><strong>الكمية</strong></td>
           <td nowrap="nowrap" width="10%" class="smallmobile" style="text-align:center;background-color:#1a3282;color:#fff;border:1px solid #cccccc;padding:10px"><strong>السعر</strong></td>
         </tr>
         <?php $countt="0";
		       $subtotal="0";
               $freeship="1";?>
         ...
         <tr>
           <td class="hideinmobile" nowrap="nowrap" style="border:1px solid #cccccc;padding:5px;text-align:center"><?php if($cc['image']!=''){?><a href="<?php echo $cc['image']?>" rel="cart" class="inline"><img src="<?php echo $cc['image']?>" alt='<?php echo str_replace("'",'`',$cc['ptitlear'])?>' style="width:50%"></a><?php }?></td>
           <td style="border:1px solid #cccccc;padding:5px;text-align:center"><?php echo $cc['ptitlear']?>
           ...
           <?php if(($aa[2])!=''){?><div style="font-size:12px">الحجم: <?php echo $con->getcertainvalue("select * from sizes where sizesname='".$aa[2]."'","sizesnamear")?></div><?php }?></td>
           <td nowrap="nowrap" style="border:1px solid #cccccc;padding:5px;text-align:center"><?php echo $b?></td>
           <td nowrap="nowrap" style="border:1px solid #cccccc;padding:5px;text-align:left"><?php echo convert($pricee, $_SESSION['hj_language'],'ar')?></td>
         </tr>
          ...
          <tr>
           <td colspan="4" align="left">
             <table>
              ...
              <tr>
                <td>رسوم التوصيل ( شامل الضريبة ) : </td>
                <?php
                $shippingprice="0";
                ...
                <td align="left"><?php $_SESSION['shippingprice']=$shippingprice; echo convert($shippingprice, $_SESSION['hj_language'],'ar')?></td>
              </tr>
              <tr class="codholder" style="display:none">
                <td>رسوم الدفع عند الاستلام (شامل الضريبة) : </td>
                <td align="left"><?php echo convert($m_config['cashextra'], $_SESSION['hj_language'],'ar')?></td>
              </tr>
               <?php if(isset($_SESSION['voucherdiscount'])){?>
               <tr>
                <td>خصم: </td>
                <td align="left"><?php echo convert($_SESSION['voucherdiscount'], $_SESSION['hj_language'],'ar')?></td>
              </tr>
              <?php }?>
              ...
              <tr>
                <td>المجموع  الاجمالي :</td>
                <td align="left">
                
				<div id="thetotal" thevalue="<?php echo $total?>"><?php echo convert($total, $_SESSION['hj_language'],'ar')?></div></td>
              </tr>
              
              <tr>
                <td>مجموع الضريبة المدفوعة </td>
                <td id="thetotalvat" thevalue="<?php echo ($total-($total/1.05))?>" align="left"><?php echo convert($total-($total/1.05), $_SESSION['hj_language'],'ar')?> </td>
              </tr>
              </table>
        ...
        <div style="display:inline-block;margin-bottom:10px"> <input type="text" name="voucher"  id="voucher" ></div>
         ...
         <div style="display:inline-block;margin-bottom:10px"><input type="submit" value="تطبيق" id="submitorder1"  class="butt"  style="width:200px" /></div>
        ...
        <?php if($m_config['cash']=='1'){$cc=4;?>	 <input type="radio" name="ceecee" id="payment4" onClick="$j('#cc').val('4');cod(<?php echo $m_config['cashextra']?>,'add')" checked="checked" > <label for="payment4">

        الدفع عند  الاستلام سيضاف (<?php echo convert($m_config['cashextra'], $_SESSION['hj_language'],'ar')?>) على المجموع
        </label></a>
        <br><?php }?>
       <?php if($m_config['visa']=='1'){ $cc=1;?>	<input type="radio" name="ceecee" id="payment1" onClick="$j('#cc').val('1');cod(<?php echo $m_config['cashextra']?>,'remove')" checked="checked"> <label for="payment1">فيزا</label></a>
        <br><?php }?>
        <?php if($m_config['visa']=='1'){ $cc=2;?>	<input type="radio" name="ceecee" id="payment2" onClick="$j('#cc').val('2');cod(<?php echo $m_config['cashextra']?>,'remove')" checked="checked"> <label for="payment1">ماستر كارد</label></a>
        <br><?php }?>
       
         <?php if($m_config['sadad']=='1'){$cc=3;?>	<input type="radio" name="ceecee" id="payment3" onClick="$j('#cc').val('3');cod(<?php echo $m_config['cashextra']?>,'remove')" checked="checked"> <label for="payment3">سداد</label></a>
        <br><?php }?>
         <?php if($m_config['mada']=='1'){$cc=5;?>	<input type="radio" name="ceecee" id="payment5" onClick="$j('#cc').val('5');cod(<?php echo $m_config['cashextra']?>,'remove')" checked="checked"> <label for="payment5">مدى - الاختيار الأفضل و الأسرع  للدفع</label></a>
        <br><?php }?>
        
        
        <div>(لا يوجد أي رسوم على الدفع أون لاين) </div>
       ...
       <?php if($total>0){?>   
        ...
        <input type="hidden" name="cc" id="cc" value="<?php echo $cc?>">
         ...
         <div style="display:inline-block;margin-bottom:10px"><input type="submit" value="أؤكد طلبي" id="submitorder" class="butt"  style="width:200px" /></div> 
        <div style="display:inline-block;margin-bottom:10px"> <input type="button" value="الرجوع" id="" onClick="window.location='address.php'" class="butt pageshower"  style="width:200px" /></div>
