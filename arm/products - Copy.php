...
//global price search for right side menus
$globalprice='';
...
if($brand!=''){
	$brdata=$con->fetch_array($con->query("select * from brands where brnamear='".m_prepare($brand)."' or brnamear='".m_prepare($brand)."'"));
}
 ...
 $extra11='';
							  ...
							  if($s=="all"){
								  ...
								  $extrabrand.="and brandsid in (select idbrands from projects left join supplier on supplierid=pidsupplier left join category on categoryid=idcategory left join brands on brandsid=idbrands left join subcategory on subcategoryid=idsubcategory  where projects.deleted='0' and snamear='".m_prepare($s)."' )";
							  ...
							  if($brand!=""){
								  $extra1.=" and (brname='".m_prepare($brand)."' or brnamear='".m_prepare($brand)."')";
							  ...
							  if($feature!=""){
								  $extra1.=" and projectsid in(select idproject from projectsfeatures left join features on featuresid=idfeature where projectsfeatures.deleted='0' and (featuresname='".m_prepare($feature)."' or featuresnamear='".m_prepare($feature)."')  )";
                                  $extracolorfeaturetypematerialsize.=" and projectsid in(select idproject from projectsfeatures left join features on featuresid=idfeature where projectsfeatures.deleted='0' and (featuresname='".m_prepare($feature)."' or featuresnamear='".m_prepare($feature)."')  )";
                                  
                                  $material_query.=" and projectsid in(select idproject from projectsfeatures left join features on featuresid=idfeature where projectsfeatures.deleted='0' and (featuresname='".m_prepare($feature)."' or featuresnamear='".m_prepare($feature)."')  )";
                                  $type_query.=" and projectsid in(select idproject from projectsfeatures left join features on featuresid=idfeature where projectsfeatures.deleted='0' and (featuresname='".m_prepare($feature)."' or featuresnamear='".m_prepare($feature)."')  )";
                                  $color_query.=" and projectsid in(select idproject from projectsfeatures left join features on featuresid=idfeature where projectsfeatures.deleted='0' and (featuresname='".m_prepare($feature)."' or featuresnamear='".m_prepare($feature)."')  )";
                                 $size_query.=" and projectsid in(select idproject from projectsfeatures left join features on featuresid=idfeature where projectsfeatures.deleted='0' and (featuresname='".m_prepare($feature)."' or featuresnamear='".m_prepare($feature)."')  )";
                               
                               
							  }
                              if($material!=""){
								  $extra1.=" and projectsid in(select idproject from projectsmaterial left join material on materialid=idmaterial where projectsmaterial.deleted='0' and (materialname='".m_prepare($material)."' or materialnamear='".m_prepare($material)."')  )";
                                  $extracolorfeaturetypematerialsize.=" and projectsid in(select idproject from projectsmaterial left join material on materialid=idmaterial where projectsmaterial.deleted='0' and (materialname='".m_prepare($material)."' or materialnamear='".m_prepare($material)."')  )";
                                  
                                  
                               $feature_query.=" and projectsid in(select idproject from projectsmaterial left join material on materialid=idmaterial where projectsmaterial.deleted='0' and (materialname='".m_prepare($material)."' or materialnamear='".m_prepare($material)."')  )";
                               $type_query.=" and projectsid in(select idproject from projectsmaterial left join material on materialid=idmaterial where projectsmaterial.deleted='0' and (materialname='".m_prepare($material)."' or materialnamear='".m_prepare($material)."')  )";
                               $color_query.=" and projectsid in(select idproject from projectsmaterial left join material on materialid=idmaterial where projectsmaterial.deleted='0' and (materialname='".m_prepare($material)."' or materialnamear='".m_prepare($material)."')  )";
                               $size_query.=" and projectsid in(select idproject from projectsmaterial left join material on materialid=idmaterial where projectsmaterial.deleted='0' and (materialname='".m_prepare($material)."' or materialnamear='".m_prepare($material)."')  )";
                               
                               
							  }
                              if($typee!=""){
								  $extra1.=" and projectsid in(select idproject from projectstypee left join typee on typeeid=idtypee where projectstypee.deleted='0' and (typeename='".m_prepare($typee)."' or typeenamear='".m_prepare($typee)."')  )";
                                  $extracolorfeaturetypematerialsize.=" and projectsid in(select idproject from projectstypee left join typee on typeeid=idtypee where projectstypee.deleted='0' and (typeename='".m_prepare($typee)."' or typeenamear='".m_prepare($typee)."')  )";
                                  
                                 $material_query.=" and projectsid in(select idproject from projectstypee left join typee on typeeid=idtypee where projectstypee.deleted='0' and (typeename='".m_prepare($typee)."' or typeenamear='".m_prepare($typee)."')  )";
                              $feature_query.=" and projectsid in(select idproject from projectstypee left join typee on typeeid=idtypee where projectstypee.deleted='0' and (typeename='".m_prepare($typee)."' or typeenamear='".m_prepare($typee)."')  )";
                               $color_query.=" and projectsid in(select idproject from projectstypee left join typee on typeeid=idtypee where projectstypee.deleted='0' and (typeename='".m_prepare($typee)."' or typeenamear='".m_prepare($typee)."')  )";
                               $size_query.=" and projectsid in(select idproject from projectstypee left join typee on typeeid=idtypee where projectstypee.deleted='0' and (typeename='".m_prepare($typee)."' or typeenamear='".m_prepare($typee)."')  )";
                               
							  }
                               if($colors!=""){
								  $extra1.=" and projectsid in(select idproject from projectscolors left join colors on colorsid=idcolors where projectscolors.deleted='0' and (colorname='".m_prepare($colors)."' or colornamear='".m_prepare($colors)."')  )";
                                  $extracolorfeaturetypematerialsize.=" and projectsid in(select idproject from projectscolors left join colors on colorsid=idcolors where projectscolors.deleted='0' and (colorname='".m_prepare($colors)."' or colornamear='".m_prepare($colors)."')  )";
                                  
                                   $material_query.=" and projectsid in(select idproject from projectscolors left join colors on colorsid=idcolors where projectscolors.deleted='0' and (colorname='".m_prepare($colors)."' or colornamear='".m_prepare($colors)."')  )";
                              $feature_query.=" and projectsid in(select idproject from projectscolors left join colors on colorsid=idcolors where projectscolors.deleted='0' and (colorname='".m_prepare($colors)."' or colornamear='".m_prepare($colors)."')  )";
                               $type_query.=" and projectsid in(select idproject from projectscolors left join colors on colorsid=idcolors where projectscolors.deleted='0' and (colorname='".m_prepare($colors)."' or colornamear='".m_prepare($colors)."')  )";
                               $size_query.=" and projectsid in(select idproject from projectscolors left join colors on colorsid=idcolors where projectscolors.deleted='0' and (colorname='".m_prepare($colors)."' or colornamear='".m_prepare($colors)."')  )";
                               
							  }
                               if($sizes!=""){
								  $extra1.=" and projectsid in(select idproject from projectssizes left join sizes on sizesid=idsizes where projectssizes.deleted='0' and (sizesname='".m_prepare($sizes)."' or sizesnamear='".m_prepare($sizes)."')  )";
                                  $extracolorfeaturetypematerialsize.=" and projectsid in(select idproject from projectssizes left join sizes on sizesid=idsizes where projectssizes.deleted='0' and (sizesname='".m_prepare($sizes)."' or sizesnamear='".m_prepare($sizes)."')  )";
                                  
                                 $material_query.=" and projectsid in(select idproject from projectssizes left join sizes on sizesid=idsizes where projectssizes.deleted='0' and (sizesname='".m_prepare($sizes)."' or sizesnamear='".m_prepare($sizes)."')  )";
                              $feature_query.=" and projectsid in(select idproject from projectssizes left join sizes on sizesid=idsizes where projectssizes.deleted='0' and (sizesname='".m_prepare($sizes)."' or sizesnamear='".m_prepare($sizes)."')  )";
                               $type_query.=" and projectsid in(select idproject from projectssizes left join sizes on sizesid=idsizes where projectssizes.deleted='0' and (sizesname='".m_prepare($sizes)."' or sizesnamear='".m_prepare($sizes)."')  )";
                               $color_query.=" and projectsid in(select idproject from projectssizes left join sizes on sizesid=idsizes where projectssizes.deleted='0' and (sizesname='".m_prepare($sizes)."' or sizesnamear='".m_prepare($sizes)."')  )";
...
<link rel="shortcut icon" href="favicon.ico">
		...
		<!-- Custom CSS -->
		<link rel="stylesheet" href="../css/style-layout11ar.css?v=4">
		<!-- Icon Fonts  -->
...
<div class="breadcrumbs">
			<div class="container">
				<ol class="breadcrumb breadcrumb--ys pull-right">
					<li class="home-link"><a href="index" class="icon icon-home"></a></li>
					   <?php if(isset($brdata['brnamear'])){?>
                      <li><a href="products?brand=<?php echo $brand?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=<?php echo urlfriendlyi($feature)?>&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=<?php echo urlfriendlyi($typee)?>&sizes=<?php echo urlfriendlyi($sizes)?>&colors=<?php echo urlfriendlyi($colors)?>"><?php echo $brdata['brnamear']?></a></li>
                      <?php }?>
                      <?php if(isset($sdata['snamear'])){?>
                      <li><?php echo $sdata['snamear']?></li>
                      <?php }?>
                      <?php if(isset($cdata['bnamear'])){?>
                      <li><?php echo $cdata['bnamear']?></li>
                      <?php }?>
                      <?php if(isset($subcategorydata['subcategorynamear'])){?>
                      <li><?php echo $subcategorydata['subcategorynamear']?></li>
                      <?php }?>
				</ol>
			</div>
		</div>
		...
		<!-- CONTENT section -->
		<div id="pageContent">
        <?php if(isset($sdata['supplierimagear']) && $sdata['supplierimagear']!=''){
            if($sdata['bannerlinkar1']!='' && $sdata['bannerlinkar1']!='0' ){
                $link=$sdata['bannerlinkar1'];
              }else{
                $link="#";
                }?>
                        <div class="container">
                    ...
                    <div class="banner-slider banner-slider-button">
							<div>
								<a href="<?php echo $link?>" class="banner zoom-in font-size-responsive">
									<span class="figure">
										<img src="../uploads/supplier/<?php echo $sdata['supplierimagear']?>" alt=""/>
									</span>
								</a>
							</div>
                            <?php if(isset($sdata['supplierimagear1']) && $sdata['supplierimagear1']!=''){
                                if($sdata['bannerlinkar2']!='' && $sdata['bannerlinkar2']!='0' ){
                                    $link=$sdata['bannerlinkar2'];
                                  }else{
                                    $link="#";
                                    }?>
                            <div>
								<a href="<?php echo $link?>" class="banner zoom-in font-size-responsive">
									<span class="figure">
										<img src="../uploads/supplier/<?php echo $sdata['supplierimagear1']?>" alt=""/>
									</span>
								</a>
							</div>
                            <?php }?>
                             <?php if(isset($sdata['supplierimagear2']) && $sdata['supplierimagear2']!=''){
                                if($sdata['bannerlinkar3']!='' && $sdata['bannerlinkar3']!='0' ){
                                    $link=$sdata['bannerlinkar3'];
                                  }else{
                                    $link="#";
                                    }?>
                            <div>
								<a href="<?php echo $link?>" class="banner zoom-in font-size-responsive">
									<span class="figure">
										<img src="../uploads/supplier/<?php echo $sdata['supplierimagear2']?>" alt=""/>
									</span>
								</a>
							</div>
                            <?php }?>
           ...
           <div class="container" style="margin-top:20px;margin-bottom:40px;">
					<div class="row">
						<div class="brands-carousel" style="margin-top:0px">
                         <?php $brr=$con->query("select * from brands where deleted='0' and bractive='Yes' and brandsimages!='' and brandsid in (select idbrands from projects left join supplier on supplierid=pidsupplier where projects.deleted='0' and snamear='".m_prepare($s)."' )   order by brpriority asc,brnamear asc");
                         
                         
						while($br=$con->fetch_array($brr)){?>
							<div style=""><a href="products?s=all&c=&min_price=&max_price=&orderby=&brand=<?php echo urlfriendlyi($br['brnamear'])?>">
                            <img src="../uploads/brands_thumb/<?php echo $br['brandsimages']?>" alt="<?php echo $br['brnamear']?>" style="border:1px solid black"></a></div>
                            <?php }?>
						</div>
					</div>
				</div>
        ...
        <?php }?>
         ...
         <input type="hidden" name="typee" value="<?php echo ($typee)?>">
					<aside class="col-md-4 col-lg-3 col-xl-2" id="leftColumn">
						<a href="#" class="slide-column-close visible-sm visible-xs"><span class="icon icon-chevron_left"></span>الرجوع</a>
							...
							<div class="filters-row__select">
								...
								<div class="select-wrapper">
									<select name="orderby" onChange="window.location='<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($c)?>&subcategory=<?php echo urlfriendlyi($subcategory)?>&min_price=<?php echo $min_price?>&max_price=<?php echo $max_price?>&orderby='+this.value+'&brand=<?php echo urlfriendlyi($brand)?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=<?php echo urlfriendlyi($feature)?>&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=<?php echo urlfriendlyi($typee)?>&sizes=<?php echo urlfriendlyi($sizes)?>&colors=<?php echo urlfriendlyi($colors)?>&searchh=<?php echo $searchh?>'"  class="select--ys sort-position">
											<option value="name"  <?php if($orderby=='name'){?>selected='selected'<?php }?>>الفرز الافتراضى</option>
                                                <option value="date" <?php if($orderby=='date'){?>selected='selected'<?php }?> >الأحدث</option>
                                                <option value="price_low" <?php if($orderby=='price_low'){?>selected='selected'<?php }?> >السعر من الارخص للاعلى</option>
                                                <option value="price_high" <?php if($orderby=='price_high'){?>selected='selected'<?php }?> >السعر الاعلى الى الادنى</option>
										</select>
								</div>
						...
						<div class="collapse-block open">
							<h4 class="collapse-block__title ">الفئة</h4>
							<div class="collapse-block__content">
                                ...
                                <li <?php if($c=='all'){?>class="active"<?php }?>><a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi('')?>&subcategory=&min_price=&max_price=&orderby=<?php echo $orderby?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&searchh=<?php echo $searchh?>&display_mode=<?php echo $display_mode?>">الكل</a></li>
                                <?php $gg=$con->query("select * from category where deleted='0' and bactive='Yes' and (bnamear!='New Arrivals') $catex  $extracategory order by bpriority asc,bnamear asc");
										while($g=$con->fetch_array($gg)){?>
									<li <?php if($c==$g['bnamear']){?>class="active open"<?php }?>>
										<a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($g['bnamear'])?>&subcategory=&min_price=&max_price=&orderby=<?php echo $orderby?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&searchh=<?php echo $searchh?>"><?php echo $g['bnamear']?></a><span class="expander"></span>
										<ul>
										...
										while($gc=$con->fetch_array($ggc)){?>
											<li <?php if($c==$gc['subcategorynamear']){?>class="active"<?php }?>>
                                            <a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($g['bnamear'])?>&subcategory=<?php echo urlfriendlyi($gc['subcategorynamear'])?>&min_price=&max_price=&orderby=<?php echo $orderby?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&searchh=<?php echo $searchh?>"><?php echo $gc['subcategorynamear']?> (<?php echo $gc['countt']?>)</a></li>
                                            <?php }?>
										</ul>
									</li>
                                    <?php }?>
						...
						<div class="collapse-block open">
							<h4 class="collapse-block__title">السعر</h4>
							<div class="collapse-block__content">
                                ...
                                <input type="hidden" name="s" value="<?php echo urlfriendlyi($s)?>">
                                 ...
                                 <input type="hidden" name="type" value="<?php echo ($type)?>">
									...
									<div class="price-input">
										<button type="submit" class="btn btn--ys btn--md">فلتر</button>
									</div>
								</form>
							</div>
						</div>
				...
				while($g=$con->fetch_array($gg)){?>
                        ...
                        if($con->num_rows($typeecheck)!=0){?>
							...
							<h4 class="collapse-block__title">النوع</h4>
							<div class="collapse-block__content" style="<?php if($typee!=''){?>display:block<?php }?>">
								<ul class="simple-list">
                                <li <?php if($typee==''){?>class="active"<?php }?>><a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($c)?>&subcategory=<?php echo urlfriendlyi($subcategory)?>&min_price=<?php echo $min_price?>&max_price=<?php echo $max_price?>&orderby=<?php echo $orderby?>&brand=<?php echo $brand?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=<?php echo urlfriendlyi($feature)?>&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=&sizes=<?php echo urlfriendlyi($sizes)?>&colors=<?php echo urlfriendlyi($colors)?>&searchh=<?php echo $searchh?>&display_mode=<?php echo $display_mode?>">الكل</a></li>
                                <?php $gg=$con->query("select *,count(*) as countt from projectstypee left join typee on typeeid=idtypee left join projects on idproject=projectsid left join supplier on supplierid=pidsupplier left join category on categoryid=idcategory left join subcategory on subcategoryid=idsubcategory left join brands on brandsid=idbrands where projects.deleted='0' $type_query and (prodactive='Yes' or prodactive='OOS') group by typeeid order by typeepriority asc,typeenamear asc");
				while($g=$con->fetch_array($gg)){?>
									  <li <?php if($typee==$g['typeenamear']){?>class="active"<?php }?>><a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($c)?>&subcategory=<?php echo urlfriendlyi($subcategory)?>&min_price=<?php echo $min_price?>&max_price=<?php echo $max_price?>&orderby=<?php echo $orderby?>&brand=<?php echo $brand?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=<?php echo urlfriendlyi($feature)?>&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=<?php echo urlfriendlyi($g['typeenamear'])?>&sizes=<?php echo urlfriendlyi($sizes)?>&colors=<?php echo urlfriendlyi($colors)?>&searchh=<?php echo $searchh?>"><?php echo $g['typeenamear']?> (<?php echo $g['countt']?>)
                                        </a></li>
                                    <?php }?>
								</ul>
							</div>
                        ...
                        if($con->num_rows($featurecheck)!=0){?>
							...
							<h4 class="collapse-block__title">المميزات</h4>
							<div class="collapse-block__content" style="<?php if($feature!=''){?>display:block<?php }?>">
								<ul class="simple-list">
                                <li <?php if($feature==''){?>class="active"<?php }?>><a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($c)?>&subcategory=<?php echo urlfriendlyi($subcategory)?>&min_price=<?php echo $min_price?>&max_price=<?php echo $max_price?>&orderby=<?php echo $orderby?>&brand=<?php echo $brand?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=<?php echo urlfriendlyi($typee)?>&sizes=<?php echo urlfriendlyi($sizes)?>&colors=<?php echo urlfriendlyi($colors)?>&searchh=<?php echo $searchh?>&display_mode=<?php echo $display_mode?>">الكل</a></li>
                                <?php $gg=$con->query("select *,count(*) as countt from projectsfeatures left join features on featuresid=idfeature left join projects on idproject=projectsid left join supplier on supplierid=pidsupplier left join category on categoryid=idcategory left join subcategory on subcategoryid=idsubcategory left join brands on brandsid=idbrands where projects.deleted='0' $feature_query and featuresactive='Yes' and (prodactive='Yes' or prodactive='OOS') group by featuresid order by featurespriority asc,featuresnamear asc");
				while($g=$con->fetch_array($gg)){?>
									  <li <?php if($feature==$g['featuresnamear']){?>class="active"<?php }?>><a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($c)?>&subcategory=<?php echo urlfriendlyi($subcategory)?>&min_price=<?php echo $min_price?>&max_price=<?php echo $max_price?>&orderby=<?php echo $orderby?>&brand=<?php echo $brand?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=<?php echo urlencode(urlfriendlyi($g['featuresnamear']))?>&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=<?php echo urlfriendlyi($typee)?>&sizes=<?php echo urlfriendlyi($sizes)?>&colors=<?php echo urlfriendlyi($colors)?>&searchh=<?php echo $searchh?>"><?php echo $g['featuresnamear']?> (<?php echo $g['countt']?>)
                                        </a></li>
                                    <?php }?>
								</ul>
							</div>
				...
				while($g=$con->fetch_array($gg)){?>
									  <li <?php if($sizes==$g['sizesnamear']){?>class="active"<?php }?>><a href="<?php echo $_SERVER['PHP_SELF']?>?s=<?php echo urlfriendlyi($s)?>&c=<?php echo urlfriendlyi($c)?>&subcategory=<?php echo urlfriendlyi($subcategory)?>&min_price=<?php echo $min_price?>&max_price=<?php echo $max_price?>&orderby=<?php echo $orderby?>&brand=<?php echo $brand?>&type=<?php echo $type?>&min_discount=<?php echo $min_discount?>&max_discount=<?php echo $max_discount?>&bestprice=<?php echo $bestprice?>&buygetfree=<?php echo $buygetfree?>&newarrival=<?php echo $newarrival?>&feature=<?php echo urlfriendlyi($feature)?>&lastpiece=<?php echo $lastpiece?>&material=<?php echo urlfriendlyi($material)?>&typee=<?php echo urlfriendlyi($typee)?>&sizes=<?php echo urlfriendlyi($g['sizesnamear'])?>&colors=<?php echo urlfriendlyi($colors)?>&searchh=<?php echo $searchh?>"><?php echo $g['sizesnamear']?> (<?php echo $g['countt']?>)
                                        </a></li>
                                    <?php }?>
								</ul>
							</div>
						...
						<div class="filters-row border-top-none">
							<div class="pull-right">
								<div class="filters-row__mode">
										<a href="#" class="btn btn--ys slide-column-open visible-xs visible-sm hidden-xl hidden-lg hidden-md">فلتر</a>
								...
								<div class="product">
									<div class="product__inside">
										...
										<div class="product__inside__image">
											...
											<!-- quick-view -->
                                            <a href="product?id=<?php echo $ress['projectsid']?>" onClick="window.location='product?id=<?php echo $ress['projectsid']?>'" class="quick-view"><b><span class="icon icon-visibility"></span> التفاصيل</b> </a> 
											<!-- /quick-view -->
										</div>
										<!-- /product image -->
                                        <?php echo showdiscount($ress['pdiscount'],'',"ar")?>
                                            <?php if($ress['newarrival']=='Yes'){?>
                                            <!--<div class="product__label product__label--left product__label--new"> <span>جديد</span> </div>-->
                                            <?php }?>
                                            <?php if($ress['pfreeshipping']=='1'){?>
                                            <div class="product__label product__label--left product__label--sale" style="background:none"> <span><span style="color:red;font-weight:bold;background-color:#ffffff">شحن مجاني</span> <br /><img src="../images/free.png" alt="Free Shipping" style="width:50px"></span> 
											</div>
                                            <?php }?>
										...
										<div class="product__inside__hover">
											...
											<div class="product__inside__info">
                                                ...
                                                <a href="product?id=<?php echo $ress['projectsid']?>" onClick="window.location='product?id=<?php echo $ress['projectsid']?>'"  id="addtocartholder" class=" btn btn--ys btn--xl"><span class="icon icon-shopping_basket"></span>  أضف الى السلة</a>
                                                <?php }else{?>
