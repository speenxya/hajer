	  ...
	  if($duplicate=='0'){
				$a["access_modified"]=date("Y-m-d H:i:s"); // we insert date modified here . note the table name prefix
				  ...
				  if(update("access",$a,"where accessid='".m_prepare($change)."'",$con)){
					  $msg="<font class='itsok'>كلمة السر الخاصة بك تم تغييرها بنجاح</font>";
...
<div class="breadcrumbs">
		...
		<div id="pageContent">			
            ...
            <div class="row" style="margin:0;border:1px solid #cccccc;padding-top:20px;padding-bottom:20px">
                <h3 style="padding-right:10px">إعادة تعيين كلمة السر</h3>
                <div>
              ...
              <div class="col-sm-6">
                 <div class="inputEntity"><label> كلمة السر الجديدة</label><input type="password" name="apassword" id="apassword" value="<?php echo $a_raw['apassword']?>" ></div>
                 <div id="pswd_info">
		<ul>
...
function m_submit1(){
	...
	if(pwdright!=4){
		  $j('#'+variabl).addClass("errorclass")
			  if(!$j('#'+variabl+'-error').length){
			   $j('#'+variabl).after("<div id='"+variabl+"-error' class='errorclass1'>1حقل الزامي</div>")
			  }
			 $j('#'+variabl).focus()
			 return false
	}else{
	...
	if($j('#'+variabl).val()==''){
		  $j('#'+variabl).addClass("errorclass")
			  if(!$j('#'+variabl+'-error').length){
			   $j('#'+variabl).after("<div id='"+variabl+"-error' class='errorclass1'>2حقل الزامي</div>")
			  }
			 $j('#'+variabl).focus()
			 return false
	}else{
