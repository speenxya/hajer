      ...
      if (bounds){
       map.fitBounds(bounds);
              ...
              <div class="col-sm-12">
               ...
               <input class="btn btn-primary" type="button" value="اضافة عنوان " onClick="window.location='addresses'">
               <br /><br />
              </div>
               <div class="col-sm-12">
              <?php $phh=$con->query("select * from addresses left join access on accessid=idaccess where idaccess='".$_SESSION['hj_id']."'");
			   while($ph=$con->fetch_array($phh)){?>
                 <div><?php echo $ph['addressesname']?>
                  (<a href="addresses?code=<?php echo $ph['addressesid']?>">تصحيح</a> -  <a href="editprofile?deleteaddress=<?php echo $ph['addressesid']?>" onClick="return confirm('هل أنت متأكد من الحذف؟')">حذف</a>)</div>
               <?php }?>
              </div>
