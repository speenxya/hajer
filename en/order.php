...
$stoporder=0;
 foreach($bas->get_cart() as $a=>$b){
				 $aa=explode(":::",$a);
				 $dat=$con->getcertainvalue("select * from projects left join category on categoryid=idcategory left join supplier on supplierid=idsupplier where projectsid='".$aa[0]."'",array("pcode"=>"pcode","projectsid"=>"projectsid","ptitle"=>"ptitle","pprice".getptype()=>"pprice".getptype(),"pcost"=>"pcost","pspecialprice".getptype()=>"pspecialprice".getptype(),"bname"=>"bname","sname"=>"sname"));
									...
									$pquantity=$dat['pquantity']-$b;
									 ...
									 //send mail if quantity<1
											// if($pquantity<=$dat['pminquantity']){
												// $body="<img src='".$m_rooturl."images/logo.png' <br><br> ".$dat['ptitle']." is nearly out of stock.<br><br>
