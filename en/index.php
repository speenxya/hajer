								...
								<span class="figure">
									...
									<span class="figcaption">
										<span class="block-table">
											...
											<?php if($mainad['matitlee']!=''){?>	<span class="block text-uppercase font-bold  font-size40"><?php echo $mainad['matitlee']?></span><?php }?>
											<?php if($mainad['descc']!=''){?>	<span class="block text-uppercase top-indent-sm1 font-size22"><?php echo $mainad['descc']?></span><?php }?>
     ...
     $res=$con->query($query);
            ...
            <div class="content">
				<div class="container">
					...
					<div class="title-with-button">
						<div class="carousel-products__button pull-right"> <span class="btn-prev"></span> <span class="btn-next"></span> </div>
						<h2 class="text-center text-uppercase title-under title-under-color2"><a href="products?type=promotion">Promotions</a></h2>
					</div>
					<!-- /title --> 
					<!-- carousel -->
                    <div class="carousel-products row" id="carousel9">
...
<?php echo gettop($ress)?>
         ...
         <?php $query="select * from news  where deleted='0' and (active='Yes') order by datee desc,ntitle asc limit 20";
							 //echo $query; 
                             $res=$con->query($query);
													   $n=0;
                                                       if($con->num_rows($res)!=0){?>   
				...
				<div class="container">
					<div class="row">
						<div class="brands-carousel">
                         <?php $brr=$con->query("select * from brands where deleted='0' and brandsimages!='' and bractive='Yes' order by brpriority asc,brname asc");
						while($br=$con->fetch_array($brr)){?>
							<div style="height:120px"><a href="products?s=all&c=&min_price=&max_price=&orderby=&brand=<?php echo urlfriendlyi($br['brname'])?>">
                            <img src="../uploads/brands_thumb/<?php echo $br['brandsimages']?>" alt="<?php echo $br['brname']?>"></a></div>
                            <?php }?>
						</div>
					</div>
				</div>
				...
				<div class="container-fluid box-baners">
					<div class="row">
						<div class="banner-carousel-1">
							...
							while($mainad=$con->fetch_array($maina)){?>
								<!-- banner -->
								<a href="<?php echo $mainad['url']?>" class="banner banner-md zoom-in" style="background:url(../uploads/background/<?php echo $mainad['background']?>?v=1)">
									<span class="figure">
										<span class="figcaption text-center">
											<span class="block-table">
												<span class="block-table-cell">
													<span class="block font-size90 text-uppercase font-light"><?php echo $mainad['matitlee']?></span>
													<?php if($mainad['url']!='#'){?><span class="btn btn--border btn--xl">Shop now!</span><?php }?>
												</span>
											</span>
										</span>
									</span>
                            ...
                            <?php $maina=$con->query("select * from mainad where deleted='0' and mposition='footer 3'  order by priority asc limit 1");?>
                                <?php
							while($mainad=$con->fetch_array($maina)){?>
								<a href="<?php echo $mainad['url']?>" class="banner banner-md bg-yellow zoom-in" style="background:url(../uploads/background/<?php echo $mainad['background']?>?v=1)">
									<span class="figure">
										<span class="figcaption text-center">
											<span class="block-table">
												<span class="block-table-cell">
                            ...
                            <?php $maina=$con->query("select * from mainad where deleted='0' and mposition='footer 4'  order by priority asc limit 1");?>
                                <?php
							while($mainad=$con->fetch_array($maina)){?>
								<a href="<?php echo $mainad['url']?>" class="banner banner-md bg-green-light zoom-in" style="background:url(../uploads/background/<?php echo $mainad['background']?>?v=1)">
									<span class="figure">
										<span class="figcaption text-center">
											<span class="block-table">
												<span class="block-table-cell">
